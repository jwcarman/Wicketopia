...
public class FileCfgPropertyDecorator implements
		MetaDataDecorator<PropertyMetaData> {
	...
	public void decorate(PropertyMetaData propertyMetaData) {
		...
		Properties properties = propertyMetaData.getBeanMetaData()
				.getFacet(FileCfgFacet.FACET_KEY).getProperties();
		if (properties != null) {
			for (Entry<Object, Object> e : properties.entrySet()) {
				if (propertyMetaData.getPropertyDescriptor().getName()
						.equals(e.getKey().toString())) {
					configureProperty(facet, e.getKey().toString(), e
							.getValue().toString());
				}
			}
		}
	...
	private void configureProperty(WicketopiaPropertyFacet facet,
	...
	private META_DATA_TYPE decodeItem(String item) {
		...
		throw new IllegalArgumentException("Invalid property specification: "
				+ item);
	}
	// ----------------------------------------------------------------------------------------------------------------------
	// Other Methods
